...
func TestVerityMetadata(t *testing.T) {
	...
	reader, _, rootHash, err := MakeErofs(tempdir, rootfs, nil, VerityMetadataPresent)
	...
	assert.NoError(err)
	erofsFile := path.Join(tempdir, "foo.erofs")
	err = os.WriteFile(erofsFile, content, 0600)
	assert.NoError(err)

	sblock, err := readSuperblock(erofsFile)
	assert.NoError(err)
	...
	cmd := exec.Command("veritysetup", "verify", erofsFile, erofsFile, rootHash,
	...
	// our small erofs file, because the stuff at the end of the verity
	// block is unused.
	const bytesToFlip = 2
	const flipAtOffset = -4087

	f, err := os.OpenFile(erofsFile, os.O_RDWR, 0644)
	assert.NoError(err)
	defer f.Close()
