...
func appendVerityData(file string) (string, error) {
	...
	// we expect mkerofs to have padded the file to the nearest 4k
...
func GuestMount(squashFile string, mountpoint string) error {
	...
	cmd, err := erofsFuse(squashFile, mountpoint)
	if err != nil {
		return err
	}
...
func Mount(erofs, mountpoint, rootHash string) error {
	if !amHostRoot() {
		return GuestMount(erofs, mountpoint)
	}
	err := HostMount(erofs, mountpoint, rootHash)
	if err == nil || rootHash != "" {
		return err
	}
	return GuestMount(erofs, mountpoint)
...
func HostMount(erofs string, mountpoint string, rootHash string) error {
	fi, err := os.Stat(erofs)
	if err != nil {
		return errors.WithStack(err)
	}

	sblock, err := readSuperblock(erofs)
	if err != nil {
		return err
	}
	...
	name := verityName(path.Base(erofs))
	...
	if rootHash != "" {
		...
		if err != nil {
			...
			loopDev, err = losetup.Attach(erofs, 0, true)
			if err != nil {
				return errors.WithStack(err)
			}
	...
	err = errors.WithStack(unix.Mount(mountSourcePath, mountpoint, "erofs", unix.MS_RDONLY, ""))
	if err != nil {
		if verityDevice != nil {
